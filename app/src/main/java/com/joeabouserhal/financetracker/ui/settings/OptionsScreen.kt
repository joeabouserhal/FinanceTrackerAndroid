...
private fun PageRow(label: String, onClick: () -> Unit) {
  ...
  Row(
    Modifier
      ...
      .drawBehind {
        drawLine(
          color = spec.border.copy(alpha = 0.45f),
    ...
    Box(Modifier.size(8.dp).background(spec.accent))
    Spacer(Modifier.width(12.dp))
    Text(label, style = MaterialTheme.typography.labelLarge, color = spec.ink, modifier = Modifier.weight(1f))
    Icon(
...
private fun SectionTitle(text: String) {
  ...
  Row(
    ...
    Box(Modifier.weight(1f).height(1.dp).background(spec.border.copy(alpha = 0.4f)))
  }
}
