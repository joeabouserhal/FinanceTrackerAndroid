...
Confirm that `local.properties` exists in the repository root and contains a valid `sdk.dir`. Also install Android SDK 36 through Android Studio's SDK Manager.

### Gradle uses the wrong Java version

Set the Gradle JDK to 17 in Android Studio. From a terminal, verify that `JAVA_HOME` and `java -version` point to a JDK 17 installation.

### The app starts in guest-only mode

Check that `supabaseUrl` and `supabaseAnonKey` are both present and valid, then rebuild the app. Build configuration values are compiled into the APK, so editing `local.properties` alone does not update an already installed build.

### A device is not detected

Enable Developer options and USB debugging on the device, accept its authorization prompt, and run `adb devices`. A physical device should appear with the state `device`, not `unauthorized` or `offline`.

### A build behaves as if configuration is stale
...
If the installed app still contains an older build, reinstall it with `installDebug` or `adb install -r`.

## License

No license file is currently included. Unless a license is added, the source remains subject to the repository owner's default copyright rights.
